...
namespace Thyt.TiPLM.CLT.TiModeler
{
    public partial class FrmMain : Form
        ...
        public ContextMenu BuildMenu(string menuName, TreeNode node) {
            ...
            switch (menuName) {
                ...
                case "CustomException":
                    ...
                    goto Label_380B;
                //2018/08/17 added by kexp 添加过程流程图右键菜单
                case "PROFLOWCHART":
                    item = new MenuItemEx("&New…", "新建(&N)…", null, null);
                    item.Click += new EventHandler(this.NewTemplate);
                    this.cmuCommon.MenuItems.Add(item);
                    ...
                    item.Click += new EventHandler(this.FreshTemplates);
                    this.cmuCommon.MenuItems.Add(item);
                    this.curContextMenu = "PROFLOWCHAR";
                    goto Label_380B;
                default:
                    goto Label_380B;
            }
        ...
        private void CreateTreeView() {
            ...
            //2018/08/17 by kexp 添加APQP业务
            //添加APQP模板
            iconIndex = ClientData.MyImageList.GetIconIndex("ICO_PPM_TMPPACKAGE");
            node = new TreeNode("APQP模板定制", iconIndex, iconIndex) {
                Tag = "APQPROOT"
            };
            this.tvwNavigator.Nodes[0].Nodes.Add(node);
            //过程流程图
            iconIndex = 0;
            TreeNode proNode = new TreeNode("过程流程图模板", -1, -1);
            iconIndex = ClientData.MyImageList.GetIconIndex("ICO_PPM_PPCARDTEMPLATE");

            UIDataModel.FillClassesTree(proNode, "PROFLOWCHART", 0, iconIndex, iconIndex);
            if (proNode.Nodes.Count > 0) {
                foreach (TreeNode pNode in proNode.Nodes) {
                    if (((DEMetaClass)pNode.Tag).Name == "PROFLOWCHART") {
                        node.Nodes.Add(pNode);
                        this.SetPPMIcons(pNode);
                        break;
                    }
                }
            }
                    ...
                    if (e.Button == MouseButtons.Right) {
                        ...
                        if (nodeAt.Tag is string) {
                            ...
                            if (parent.Tag.Equals("PPROOT")) {
                                ...
                                //2018/08/17 added by kexp 设置apqp的菜单
                            } else if (parent.Tag.Equals("APQPROOT")) {
                                menuName = "PROFLOWCHART";
                                if (((DEMetaClass)nodeAt.Tag).Name == "PROFLOWCHART") {
                                    menuName = "APQPROOT";
                                }
